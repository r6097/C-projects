...
/*
 * Bugs:
 * (1) doesn't output specific bit, and isn't in binary form either
 * 
 */
int main(void)
{
	...
	while(printf("integer bitnumber> "),
		...
		/* print the result */
		/* 
		 * problem:  shifts binary number to right equal to the bit position 
		 * but returns the number in integer form 
		 */
		printf("%d\n", (n >> bpos) & 1); 
		/*
		 * we can do bitwise and operator '&' to check for matches of 1
		 * bitwise & sets a bit to 1 if it matches, or 0 if it doesnt match
		 * 
		 * it will execute the & operator once, and since we shifted it 
		 * bpos number of times, the first bit it will check is the bit
		 * we want to print
		 */
	}
